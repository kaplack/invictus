...
export function createEventService({store, publicBaseUrl='http://127.0.0.1:3200', fileService}={}){
  ...
  const checkFiles=async(user,input)=>{const ids=[input.primaryImageFileId,input.bannerImageFileId,...(input.galleryFileIds||[]),...(input.benefits||[]).map(b=>b.imageFileId)].filter(Boolean);if(ids.length&&!fileService)throw new EventError('Configura el adaptador de archivos',409,'FEATURE_NOT_CONFIGURED');for(const id of ids)await fileService.assertOwned(id,user.id,{visibility:'public',image:true})};
  ...
  return { STATES, BENEFITS,
    ...
    async getManaged(user,id){const e=await read(id);canManage(user,e);return e;}, canManage
  };
}
