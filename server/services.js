...
export function composeServices({database:db,Prisma,files,config}) {
 ...
 const events={
  ...
  async save(actor,id,input){const user=organizer(actor);const {maxCapacity,...data}=input; if(!capacity.safeParse(maxCapacity).success)throw new AppError('Indica un cupo entre 1 y 1000000',400,'INVALID_CAPACITY');if(data.status&&!['DRAFT','PUBLISHED','CLOSED'].includes(data.status))throw new AppError('Estado no disponible',400,'INVALID_STATE');return commerce.run(null,async tx=>{const svc=eventsFor(tx.database);const event=id?await svc.update(user,id,data):await svc.create(user,data);await registrationsFor(tx.database).service.configure(user,event.id,{amountCents:0,currency:'PEN',maxCapacity,paymentRecipientId:null});return event;});},
  ...
  async publish(actor,id){return commerce.run(null,async tx=>{if(!await tx.database.eventRegistrationConfig.findUnique({where:{eventId:id}}))throw new AppError('Configura el cupo primero',409,'NOT_CONFIGURED');return eventsFor(tx.database).publish(organizer(actor),id);});},
 ...
 return {events,profiles,shop,checkout,payments,users,recipients:createRecipientService({store:commerce,fileService:files,resolvePermissions})};
}
